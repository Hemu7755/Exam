...
<body>
    ...
    <center>
        <div class="display-2 m-5">
            <?php
            $data = mysqli_query($conn, "SELECT student_name FROM student_login WHERE student_id='$userid'");

            if ($data) {
                $row = mysqli_fetch_assoc($data);
                $studentName = $row['student_name'];
            ?>
                <p>Welcome <?php echo $studentName; ?>, start the section!</p>
            <?php
            } else {
    ...
    $userExams = mysqli_query($conn, "SELECT e_id, exam_name, subject, branch, total_marks, exam_date, time, start_time,end_time, status 
                                  FROM exam 
                                  WHERE status='activate' 
                                  AND branch = '$userBranch' 
                                  AND DATE(start_time) = CURDATE()
                                  AND TIME(start_time) > CURTIME()");

    while ($userTransaction = mysqli_fetch_assoc($userExams)) {
        echo generateCard($userTransaction);
    }
    ?>
</div>
<script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            cards.forEach(function(card) {
                ...
                if (remainingTime > 0) {
                    ...
                    displayMessage('Your time for Today\'s test has expired. You are safe Now.');
                }

                var takeTestButton = card.querySelector('.take-test-button');
            ...
            <div class='card' data-remaining-time='$remainingTime'>
                <div class='card-body'>
                    ...
                    <p class='card-text'>
                        ...
                        <strong>Exam Time:</strong> $transaction[time]<br>
                        <strong>Exam Start:</strong> $transaction[start_time]<br>
                    ...
                    <center>
                        <button class='btn btn-primary take-test-button' disabled>Take Test</button>
                    </center>
...
?>
    ...
    <center>
        <form action="" method="post">
            <button class="but" style="--clr:#ff1867" type="submit" id="terms" name="terms"><span>START</span><i></i></button>
        </form>
    </center>
